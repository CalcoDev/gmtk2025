...
func prepare_dialogue(filepath: String) -> Array:
	...
	file.close()
	var retarr = prepare_dialogue_string(filepath, raw_file)
	return retarr

func prepare_dialogue_string(id: String, text: String) -> Array:
	var lexed_nodes_dict := DialogueLexer.lex_nodes(text)
	_f[id] = []
	for node: DialogueLexer.NodeData in lexed_nodes_dict.values():
		...
		_f[id].append(node.title)

	_n[id] = lexed_nodes_dict.keys()
	return get_prepared_dialogue_nodes(id)
...
func unprepare_undialogue(filepath: String) -> void:
	for f in _f[filepath]:
		_dialogue_nodes_tokens.erase(f)
...
func _start_node_coro(node_title: String) -> void:
	var should_set_question := false
	var question: DialogueLine
	...
	while current != null:
		match current.type:
			DialogueParser.Token.LINE:
				...
				if should_set_question:
					question = line.dia_line.duplicate()
					should_set_question = false
				else:
					var state := {"sum": 0}
			...
			DialogueParser.Token.OPTION:
				...
				var on_finished := func(index: int):
					state["sum"] += 1
					state["index"] = index
				var cnt := _views_run_options(question, opt.generate_dialogue_options(), on_finished)
				question = null
				while state["sum"] < cnt:
					await get_tree().process_frame
...
func _views_run_options(question: DialogueLine, options: Array[DialogueOption], on_selected: Callable) -> int:
	var s: int = 0
	for view in dialogue_views:
		if view.can_handle_options():
			view.on_run_options.emit(question, options, on_selected)
			s += 1
	return s
